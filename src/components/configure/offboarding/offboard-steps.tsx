...
import {useGetOffBoardingSteps} from '@queries/use-offboard-step';
import React, {useMemo, useState} from 'react';
import {EditOffBoardStep} from './edit-offboard-step';

export const OffboardSteps = ({offBoardId}: {offBoardId: number}) => {
  const [activeStepIndex, setActiveStepIndex] = useState<number>(-1);
  const [steps, setSteps] = useState<string[]>([]);

  const {data: offboardingSteps, isLoading} = useGetOffBoardingSteps(
    offBoardId,
    {
      onSuccess: newData => {
        if (newData?.length === 0) {
  ...
  const isCreating =
    offboardingSteps && steps.length > offboardingSteps?.length;
  ...
  const activeStep = useMemo(
    () => offboardingSteps?.find((_, i) => i === activeStepIndex),
    [activeStepIndex, offboardingSteps],
  );
  const prevOrder = offboardingSteps?.[offboardingSteps?.length - 1]?.order;
